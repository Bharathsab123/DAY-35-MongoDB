...
db.product.find().forEach(function (value){
    print("product_name : " + value.product_name)
    print("product_material : " +value.product_material)})
...
db.product.aggregate([{$group :{ _id:"$product_price",
duplicate:{$addToSet:"$_id"},
totalcount:{$sum:1}}},{$match:{totalcount:{$gt:1}}}]).forEach((val)=>{
    val.duplicate.shift();
    db.product.deleteMany({_id:{$in:val.duplicate}})
})
db.product.find()